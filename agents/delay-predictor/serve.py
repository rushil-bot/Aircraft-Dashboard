...
class PredictRequest(BaseModel):
    """Schema for a request to predict a flight delay."""
...
class PredictResponse(BaseModel):
    ...
    feature_importance_note: str = (
        "Values >= 50% probability predict a 15+ minute delay."
    )
...
async def lifespan(_app: FastAPI):
    """Lifespan manager for the FastAPI applications to load ML artifacts."""
    # pylint: disable=global-statement
    global model_instance, encoders, feature_columns

    try:
        print("Loading ML artifacts...")
        model_instance = joblib.load(MODEL_DIR / "xgboost_delay_model.joblib")
        ...
        print("Ensure train.py has been executed.")
...
def predict_delay(req: PredictRequest):
    ...
    try:
        ...
        for col, raw_val in [
            ...
            le = encoders[col]
            # If we've never seen this airport/airline            # Fallback for unseen values
            # Assuming UNKNOWN was used in training, fallback to first item otherwise.
            val = raw_val.upper()
            if val not in le.classes_:
                if "UNKNOWN" in le.classes_:
                    val = "UNKNOWN"
                else:
                    # Fallback to the most common or specific class                else:
                    # A robust system should have an Other category.
                    val = le.classes_[0]
        ...
        proba = model_instance.predict_proba(df)[0][
            1
        ]  # Probability of class 1 (Delayed)

        return PredictResponse(
            is_delayed=bool(proba >= 0.50), delay_probability=round(float(proba), 3)
        )
